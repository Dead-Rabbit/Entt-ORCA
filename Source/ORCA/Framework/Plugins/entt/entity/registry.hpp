...
class Registry {
	...
	// AZURE
	template<typename Component>
	inline const SparseSet<Entity, Component> * pool_ptr() const ENTT_NOEXCEPT {
		const auto ctype = component_family::type<Component>();
		return ctype < pools.size() ? static_cast<SparseSet<Entity, Component> *>(std::get<0>(pools[ctype]).get()) : nullptr;
	}
...
public:
    ...
    entity_type create() {
        ...
        if(available) {
			...
			//check(entity < traits_type::entity_mask); 
        }
		//UE_LOG(LogECSLG, Warning, TEXT("create %d %d %d %d %d"), entity, entity & traits_type::entity_mask, (entity & 0xfff00000) >> 20, next, available);
        return entity;
    }
    ...
    void destroy(const entity_type entity) {
        ...
        next = entt;
        ++available;
		//UE_LOG(LogECSLG, Warning, TEXT("destroy %d %d %d %d %d"), entity, entity & traits_type::entity_mask, (version & 0xfff00000) >> 20, next, available);
    }
	...
	template<typename Component>
	const Component * getptr(const entity_type entity) const ENTT_NOEXCEPT {
		//if (!valid(entity))
		//	return nullptr;
		...
		if (!valid(entity))
			return nullptr;
		const auto* poolPtr = pool_ptr<Component>();
		return poolPtr ? poolPtr->getptr(entity) : nullptr;
	}
	...
	template<typename Component>
	inline Component * getptr(const entity_type entity) ENTT_NOEXCEPT {
		return const_cast<Component *>(const_cast<const Registry *>(this)->getptr<Component>(entity));
	}
