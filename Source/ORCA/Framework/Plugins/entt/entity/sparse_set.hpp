...
// AZURE
#if defined(UE_GAME) && UE_GAME
#if PLATFORM_WINDOWS && !UE_BUILD_SHIPPING
#define ECS_THREAD_SAFE_CHECK 1
#endif
#endif

#ifdef ECS_THREAD_SAFE_CHECK
#include <cstdio>
extern void glbECSThreadSafeCheckFunc(const char* szForceCrashReason = nullptr);
#endif
...
class SparseSet<Entity> {
    ...
    class Iterator final {
            ...
            : direct{direct}, index{index}
        {
			...
			{
				char reason[1024];
				std::sprintf(reason, "SparseSet<Entity>::Iterator, construct, invalid index = %d", (int)index);
				glbECSThreadSafeCheckFunc(reason);
			}
#endif
		}
    ...
    public:
        ...
        Iterator & operator+=(const difference_type value) ENTT_NOEXCEPT {
			...
			{
				char reason[1024];
				std::sprintf(reason, "SparseSet<Entity>::Iterator, operator+=, invalid index = %d, valude = %d", (int)index, (int)value);
				glbECSThreadSafeCheckFunc(reason);
			}
#endif
        ...
        reference operator[](const difference_type value) const ENTT_NOEXCEPT {
			...
			{
				char reason[1024];
				std::sprintf(reason, "SparseSet<Entity>::Iterator, operator[], invalid index = %d, value = %d", (int)index, (int)value);
				glbECSThreadSafeCheckFunc(reason);
			}
#endif
        ...
        pointer operator->() const ENTT_NOEXCEPT {
			...
			{
				char reason[1024];
				std::sprintf(reason, "SparseSet<Entity>::Iterator, operator->(), invalid index = %d", (int)index);
				glbECSThreadSafeCheckFunc(reason);
			}
#endif
...
public:
     ...
     */
    inline bool has(const entity_type entity) const ENTT_NOEXCEPT {
     ...
     * @return True if the sparse set contains the entity, false otherwise.
     */
    inline bool fast(const entity_type entity) const ENTT_NOEXCEPT {
        const auto pos = size_type(entity & traits_type::entity_mask);
        check(pos < reverse.size());
        // testing against null permits to avoid accessing the direct vector
        return (reverse[pos] != null);
    }

	inline bool fast2(const size_type pos) const ENTT_NOEXCEPT {
		check(pos < reverse.size());
		// testing against null permits to avoid accessing the direct vector
		return (reverse[pos] != null);
	}
     ...
     */
    inline size_type get(const entity_type entity) const ENTT_NOEXCEPT {
#ifdef ECS_THREAD_SAFE_CHECK
		check(has(entity));
#else
		check(has(entity));
	...
	// AZURE
	inline const entity_type* getptr(const entity_type entity) const ENTT_NOEXCEPT {
		const auto pos = size_type(entity & traits_type::entity_mask);
		if (pos >= reverse.size())
			return nullptr;
		// testing against null permits to avoid accessing the direct vector
		const auto& ret = reverse[pos];
		return ret == null ? nullptr : &ret;
	}
    ...
    void swap(const size_type lhs, const size_type rhs) ENTT_NOEXCEPT {
        check(lhs < direct.size());
        check(rhs < direct.size());
        auto &src = direct[lhs];
        auto &dst = direct[rhs];
...
class SparseSet<Entity, Type>: public SparseSet<Entity> {
    ...
    class Iterator final {
            ...
            : instances{instances}, index{index}
        {
			...
			{
				char reason[1024];
				std::sprintf(reason, "SparseSet<Entity, Type>::Iterator, construct, invalid index = %d", (int)index);
				glbECSThreadSafeCheckFunc(reason);
			}
#endif
		}
    ...
    public:
        ...
        reference operator[](const difference_type value) const ENTT_NOEXCEPT {
			...
			{
				char reason[1024];
				std::sprintf(reason, "SparseSet<Entity, Type>::Iterator, operator[], invalid index = %d, valude - %d", (int)index, (int)value);
				glbECSThreadSafeCheckFunc(reason);
			}
#endif
        ...
        pointer operator->() const ENTT_NOEXCEPT {
			...
			{
				char reason[1024];
				std::sprintf(reason, "SparseSet<Entity, Type>::Iterator, operator->(), invalid index = %d", (int)index);
				glbECSThreadSafeCheckFunc(reason);
			}
#endif
...
public:
	...
	// AZURE
	const object_type * getptr(const entity_type entity) const ENTT_NOEXCEPT {
		const auto* indexPtr = underlying_type::getptr(entity);
		return indexPtr ? &instances[(size_type)(*indexPtr)] : nullptr;
	}
