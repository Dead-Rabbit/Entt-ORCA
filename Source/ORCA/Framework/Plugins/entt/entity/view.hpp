...
class View final {
    ...
    class Iterator {
        ...
        inline bool valid() const ENTT_NOEXCEPT {
            ...
            return pos < extent && std::all_of(unchecked.cbegin(), unchecked.cend(), [pos](const view_type *view) {
                return view->fast2(pos);
            });
        }
...
public:
	...
	/*! @brief Use for random access when ParallelFor */
	entity_type random_get(index_type index) const ENTT_NOEXCEPT {
	...
	public:
		random_indexer(unchecked_type unchecked, const entity_type * in_data, size_type in_size) ENTT_NOEXCEPT :
			unchecked{ unchecked },
			extent{ min(std::make_index_sequence<unchecked.size()>{}) },
			data_ptr(in_data),
			data_size(in_size)
		...
		inline entity_type rawget(index_type index) const ENTT_NOEXCEPT
		{
			return *(data_ptr + index);
		}

		inline bool valid(entity_type entity) const ENTT_NOEXCEPT
		{
			const auto pos = size_type(entity & traits_type::entity_mask);

			if (pos < extent && std::all_of(unchecked.cbegin(), unchecked.cend(), [pos](const view_type *view) {
				return view->fast2(pos); }))
			{
				return true;
			}
	...
	private:
		unchecked_type unchecked;
		extent_type extent;
		const entity_type * data_ptr;
		size_type data_size;
	};

	random_indexer get_random_indexer() const ENTT_NOEXCEPT
	{
		const auto *view = candidate();
		return random_indexer{ unchecked(view), view->data(), view->size() };
	}
