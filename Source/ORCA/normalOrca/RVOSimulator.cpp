...
namespace RVO {
	RVOSimulator::RVOSimulator(AActor * entryPoint_) : entryPoint_(entryPoint_), defaultAgent_(NULL), globalTime_(0.0f), kdTree_(NULL), timeStep_(0.0f)
	{
...
#pragma omp parallel for
	...
	void RVOSimulator::SetAgentActorLocation(int index)
	{
		if (agentActors.size() <= index)
			return;
		
		auto agentActor = agentActors[index];

		const auto prePos = agentActor->GetActorLocation();
		const auto pos = getAgentPosition(index);
		const auto curPos = FVector(pos.x(), pos.y(), 0);
		if ((curPos - prePos).SizeSquared() > 1)
		{
			...
			{
				// agentActor->SetActorRotation(dir.ToOrientationQuat());
	...
	void RVOSimulator::ShowDebugDraw()
	{
		auto renderWorld = AORCAEntry::get();
		for (Obstacle * obs : obstacles_) {
			auto nextObs = obs->nextObstacle_;
			const auto points = obs->point_;
			const auto nextPoint = nextObs->point_;
			
			DrawDebugLine(renderWorld->GetWorld(), FVector(points.x(), points.y(), 10),
				FVector(nextPoint.x(), nextPoint.y(), 10), FColor::Blue);
		}

		for (Agent * agent : agents_) {
			const auto pos = FVector(agent->position_.x(), agent->position_.y(), 0);
			if (agent->obstacleNeighbors_.size() > 0) {
				for (auto obsPari : agent->obstacleNeighbors_) {
					const Obstacle * obs = obsPari.second;
					auto nextObs = obs->nextObstacle_;
					const auto points = obs->point_;
					const auto nextPoint = nextObs->point_;
			
					DrawDebugLine(renderWorld->GetWorld(), FVector(points.x(), points.y(), 10),
						FVector(nextPoint.x(), nextPoint.y(), 20), FColor::Green);
				}
			}

			for (auto line : agent->orcaLines_) {
				DrawDebugLine(renderWorld->GetWorld(), pos + FVector(line.point.x(), line.point.y(), 0) + FVector(0, 0, 20),
					pos + FVector(line.point.x(), line.point.y(), 0) + FVector(line.direction.x(), line.direction.y(), 0) * 200 + FVector(0, 0, 20), FColor::Yellow
					, false, -1, 0, 5.0f);
			}
